...
public:
    ...
    {if(ind>=n) return 0; // why greater than equal to n,whay not ind==n only?
     if(dp[ind][buy]!=-1) return dp[ind][buy];
        if(buy)
            profit=max(-prices[ind]+ solve(ind+1,0,prices,dp,n,profit),0+ solve(ind+1,1,prices,dp,n,profit));
         else
            profit=max(prices[ind]+ solve(ind+2,1,prices,dp,n,profit), solve(ind+1,0,prices,dp,n,profit));
